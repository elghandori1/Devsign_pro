...
import {
  ...
  MousePointerClick, Palette, TrendingUp, PenTool,
  Image as ImageIcon, Layers, MonitorPlay, MoreHorizontal
} from "lucide-react";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    keywords: ["social media ads", "ad design", "facebook ads", "instagram creatives", "conversion design"],
  });
}
...
const getToolIcon = (toolName: string) => {
  ...
  if (name.includes("figma")) return PenTool;
  if (name.includes("photoshop")) return ImageIcon;
  if (name.includes("illustrator")) return Layers;
  if (name.includes("effects") || name.includes("video")) return MonitorPlay;
  return PenTool; // default
};
...
export default async function SocialDesignPage({ params }: Props) {
  ...
  return (
    <main dir={isRtl ? "rtl" : "ltr"} className="min-h-screen bg-background">
      ...
      {/* ── 2. OVERVIEW ── */}
      <section aria-labelledby="overview-heading" className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
      ...
      <section aria-labelledby="example-heading" className="bg-muted/10 border-y border-border overflow-hidden">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">

            {/* Left: Text & Copyright */}
            <div className="space-y-6">
              <h2 id="example-heading" className="text-2xl sm:text-3xl font-bold text-foreground">
                {data.exampleTitle}
              </h2>
              ...
              {data.exampleFeatures && (
                <ul className="space-y-3 pt-2">
                  {data.exampleFeatures.map((feature: string, i: number) => (
                    <li key={i} className="flex items-center gap-3 text-sm sm:text-base text-foreground font-medium">
            ...
            <div className="relative w-full max-w-md mx-auto lg:ml-auto bg-card rounded-2xl border border-border shadow-2xl p-4 sm:p-6 flex flex-col gap-4 transform transition-transform hover:-translate-y-1">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-sm font-bold text-foreground leading-none">YourBrand</p>
                    <p className="text-[11px] text-muted-foreground font-medium mt-1">Sponsored</p>
                  </div>
                </div>
              ...
              <div className="relative w-full aspect-square sm:aspect-[4/5] rounded-xl overflow-hidden bg-muted flex flex-col items-center justify-center border border-border/50 group">
                <div className="absolute inset-0 bg-gradient-to-br from-primary/10 to-background pointer-events-none" />

                {/* Simulated Product / Graphic */}
                <div className="relative z-10 w-24 h-24 rounded-2xl bg-background shadow-xl flex items-center justify-center mb-6 border border-border/50 group-hover:scale-105 transition-transform duration-500">
                  <ImageIcon size={40} className="text-primary/60" />
                </div>
                <h3 className="relative z-10 text-3xl font-black text-foreground uppercase tracking-tight text-center px-4 leading-none">
                  High<br /><span className="text-primary">Conversion</span>
                </h3>
              </div>
              ...
              <div className="flex items-center justify-between bg-muted/50 border border-border p-3 sm:p-4 rounded-xl">
                <div>
                  <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-wider mb-1">Limited Offer</p>
                  <p className="text-sm font-bold text-foreground">Get Started Today</p>
                </div>
      ...
      {data.process?.length > 0 && (
        <section aria-labelledby="process-heading" className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-20">
      ...
      {data.toolsList?.length > 0 && (
        <section aria-labelledby="tools-heading" className="py-12 sm:py-16 border-t border-border bg-muted/5">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <div className="text-center mb-10 sm:mb-14">
              <h2 id="tools-heading" className="text-2xl sm:text-3xl font-bold mb-3 text-foreground">
                {data.toolsTitle}
              </h2>
              <p className="text-muted-foreground max-w-xl mx-auto text-sm sm:text-base">
                {data.toolsDesc}
              ...
              {data.toolsList.map((toolName: string, i: number) => {
                ...
                return (
                  <div key={i} className="flex items-center justify-center gap-2 sm:gap-3 py-2.5 sm:py-3 px-5 sm:px-6 rounded-full border border-border bg-card hover:border-primary/40 hover:shadow-md transition-all group cursor-default">
                    ...
                    <span className="text-xs sm:text-sm font-semibold text-foreground/80 group-hover:text-foreground">{toolName}</span>
                  </div>
                );
              })}
            </div>
