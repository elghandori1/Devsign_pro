...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    keywords: ["web development Morocco", "SEO website", "Next.js developer", "responsive design", "custom website Morocco"],
  });
}
...
export default async function WebDevelopmentPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      {/* ── 3. WHAT'S INCLUDED ── */}
      {data.features?.length > 0 && (
        <section aria-labelledby="included-heading" className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16 border-t border-border">
            ...
            {data.features.map((f: string, i: number) => (
              <article key={i}
                ...
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-primary/10 flex items-center justify-center
                  ...
                  <CheckCircle2 className="w-4 h-4 sm:w-5 sm:h-5 text-primary" aria-hidden="true" />
                </div>
                <span className="text-sm sm:text-base font-medium text-foreground">{f}</span>
              </article>
            ))}
          </div>
        </section>
      ...
      {data.benefits?.length > 0 && (
        <section aria-labelledby="benefits-heading" className="bg-muted/5 border-t border-border">
              ...
              {data.benefits.map((b: { title: string; text: string }, i: number) => {
                ...
                return (
                  <article key={i}
                    ...
                    <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" aria-hidden="true" />
                    ...
                    <h3 className="font-semibold text-foreground mb-2 text-base sm:text-lg">{b.title}</h3>
                    <p className="text-sm sm:text-base text-muted-foreground leading-relaxed flex-grow">{b.text}</p>
                  </article>
                );
              })}
            </div>
      ...
      {data.process?.length > 0 && (
        <section aria-labelledby="process-heading" className="max-w-6xl mx-auto px-4 sm:px-6 py-12 sm:py-16 border-t border-border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 sm:gap-10 relative">
            ...
            <div className="hidden md:block absolute top-7 left-[16.66%] right-[16.66%] h-px bg-border" aria-hidden="true" />
            
            {data.process.map((p: { step: string; title: string; text: string }, i: number) => (
              <article key={i} className="flex flex-col items-center text-center gap-3 sm:gap-4">
                ...
                <div>
                  <h3 className="font-semibold text-foreground mb-1.5 sm:mb-2 text-base sm:text-lg">{p.title}</h3>
          ...
          <div className="grid grid-cols-2 xs:grid-cols-3 md:grid-cols-6 gap-3 sm:gap-4">
            {TECH_STACK.map((tech, i) => (
              <div key={i}
                ...
                <tech.icon className="w-5 h-5 sm:w-6 sm:h-6 text-primary group-hover:scale-110 transition-transform" aria-hidden="true" />
                <span className="text-xs sm:text-sm font-medium text-foreground text-center leading-tight">{tech.name}</span>
              </div>
            ))}
          </div>
        </div>
