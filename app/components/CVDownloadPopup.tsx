            ...
            <div className="flex max-h-[calc(100dvh-2rem)] sm:max-h-[calc(90vh-3rem)] flex-col overflow-hidden">
              ...
              <div className="mb-4 pr-10 text-center sm:mb-6 sm:pr-0">
                <div className="mb-2 inline-flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 sm:mb-3 sm:h-12 sm:w-12">
                  <svg className="h-5 w-5 text-primary sm:h-6 sm:w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      strokeWidth={2}
                ...
                <h3 id="cv-popup-title" className="text-base font-semibold text-foreground sm:text-lg">
                  {popupTitle}
                </h3>
                <p className="mt-1 text-xs text-muted-foreground sm:text-sm">{popupDescription}</p>
              </div>
              ...
              <div className="min-h-0 flex-1 space-y-2 overflow-y-auto pr-1 -mr-1 sm:space-y-3">
                {cvOptions.map((cv) => (
                  <a
                    ...
                    download
                    onClick={() => setShowCVPopup(false)}
                    className={`group flex items-center gap-3 rounded-xl border border-border p-3 transition-all touch-manipulation hover:border-primary/30 hover:bg-muted/50 active:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-offset-1 sm:gap-4 ${cv.ringColor}`}
                  >
                    <div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg text-white sm:h-10 sm:w-10 ${cv.color}`}>
                      {cv.icon}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 truncate text-xs text-muted-foreground">{cv.description}</p>
                    </div>

                    <svg className="h-4 w-4 shrink-0 text-muted-foreground transition-colors group-hover:text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <p className="pt-3 text-center text-[10px] text-muted-foreground sm:pt-4 sm:text-xs">{popupFooter}</p>
