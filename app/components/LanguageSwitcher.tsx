    ...
    <div className="relative" ref={dropdownRef}>
      <nav className="sr-only" aria-label="Languages">
        {otherLocales.map((loc) => (
          <Link
            key={loc}
            href={localePath(pathname, loc)}
            hrefLang={localeToBcp47(loc)}
          >
            {localeLabels[loc]}
          </Link>
        ))}
      ...
      <div
        ...
        role="listbox"
        hidden={!langOpen}
        className="absolute top-full mt-2 right-0 min-w-16 bg-[#050505] text-zinc-300 rounded-md z-50 border border-white/10 overflow-hidden"
      >
        {otherLocales.map((loc) => (
          <Link
            key={loc}
