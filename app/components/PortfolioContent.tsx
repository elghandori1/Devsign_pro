...
export default function PortfolioContent({
  ...
  const ctaTitle =
    ui.ctaTitle ?? "Got a project idea? Let's make it happen.";
  ...
  return (
    ...
    >
      ...
      >
        ...
        />
        <div
          className="absolute -top-32 -left-32 w-96 h-96 bg-primary/20 rounded-full blur-3xl opacity-40 pointer-events-none"
          aria-hidden="true"
        />
        <div
          className="absolute -bottom-16 right-0 w-72 h-72 bg-primary/10 rounded-full blur-3xl opacity-30 pointer-events-none"
          aria-hidden="true"
        />
            ...
            {(["professional", "personal", "academic"] as ProjectType[]).map(
              (type) => (
                <Link
                  ...
                  aria-label={`${typeLabels[type]}: ${counts[type]} projects`}
                >
                  <span
      ...
      {/* ── Filter Bar ── */}
      <nav
        aria-label={ui.filterBy ?? "Project filters"}
        className="sticky top-0 z-20 bg-background/90 backdrop-blur-md border-b border-border"
      >
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3">
            ...
            <div
              className="hidden sm:block w-px h-8 bg-border shrink-0 mx-2"
              aria-hidden="true"
            />
              ...
              {filters.map((f) => {
                ...
                return (
                  ...
                  >
                    {f.id !== "all" && (
                      <span
                        className={`w-1.5 h-1.5 rounded-full shrink-0 ${getBadge(f.id).dot}`}
                        aria-hidden="true"
                      />
                    )}
                    <span>{f.label}</span>
            ...
            <span
              className="hidden lg:block text-xs text-muted-foreground ms-auto shrink-0"
              aria-live="polite"
            >
              ...
              {filteredProjects.map((project, i) => (
                ...
                >
                  <Link
                    ...
                    aria-label={`${data.viewCase}: ${project.title}`}
                  >
                    {/* Image */}
                    <div className="relative h-52 overflow-hidden bg-gradient-to-br from-muted to-muted/80 shrink-0">
                      <div
                        className="absolute inset-0 scale-110 blur-xl opacity-30 pointer-events-none"
                        aria-hidden="true"
                      >
                        <Image
                          src={project.image}
                      ...
                      <div
                        className="absolute inset-0 bg-gradient-to-t from-black/60 via-black/20 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 z-10"
                        aria-hidden="true"
                      />
                      ...
                      {/* Type badge */}
                      <div
                        className={`absolute top-3 ${isRtl ? "right-3" : "left-3"} z-20`}
                      >
                    ...
                    <div className="relative z-10 flex flex-col flex-1 p-6 gap-4 bg-card">
                      ...
                      <div
                        className="h-px bg-gradient-to-r from-border via-border to-transparent"
                        aria-hidden="true"
                      />
                    ...
                    {/* Bottom accent */}
                    <div
                      className="absolute inset-x-0 bottom-0 h-1 bg-gradient-to-r from-primary via-primary/80 to-primary scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-left rounded-b-full"
                      aria-hidden="true"
                    />
                    ...
                    <div
                      ...
                      aria-hidden="true"
                    >
                      <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-bl from-primary/15 to-transparent rotate-45 translate-x-10 -translate-y-10 group-hover:translate-x-8 group-hover:-translate-y-8 transition-transform duration-500" />
                    </div>
                  </Link>
                </article>
              ))}
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-14 sm:py-20">
            ...
            >
              {data.whyChooseUs.reasons.map((reason, index) => {
                const Icon =
                  WHY_CHOOSE_ICONS[index % WHY_CHOOSE_ICONS.length];
                return (
                  <article
                    key={reason.title}
                  ...
                  >
                    <div
                      className="absolute top-0 inset-x-0 h-px bg-gradient-to-r from-transparent via-primary/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity rounded-t-2xl"
                      aria-hidden="true"
                    />
                    <div
                      className="w-11 h-11 rounded-xl bg-primary/10 flex items-center justify-center mb-5 group-hover:bg-primary/20 transition-colors"
                      aria-hidden="true"
                    >
      ...
      >
        <div className="relative rounded-3xl border border-border bg-card overflow-hidden p-8 sm:p-14 text-center">
          ...
          />
          <div
            className="absolute -top-20 left-1/2 -translate-x-1/2 w-80 h-80 bg-primary/10 rounded-full blur-3xl pointer-events-none"
            aria-hidden="true"
          />
          ...
          <div className="relative">
            <h2
              ...
              className="text-2xl sm:text-3xl lg:text-4xl font-bold mb-4 max-w-2xl mx-auto leading-tight"
            >
              {ctaTitle}
            </h2>
